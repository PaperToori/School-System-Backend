...
app.post('/lessons/', async ({ body, set }) => {
    ...
    let check = false;
    let message = "Error:\n";
    if (! await Classroom.exists    ({name : newLesson.classroom})) 
    { check = true;  message += "Ghost classroom detected\n"; }
    if (! await Course.exists       ({name : newLesson.course}))    
    { check = true; message += "Ghost course detected\n"; } 
    if (! await Group.exists        ({name : newLesson.class}))     
    { check = true; message += "Ghost class detected\n"; }
    if (! await Teacher.exists      ({name : newLesson.teacher}))   
    { check = true; message += "Ghost teacher detected\n"; }
...
app.post('/students/', async ({ body, set }) => {
    ...
    // Vibecheck
    if (! await Course.exists({ name: newStudent.course })) 
...
app.delete('/lessons/', async ({ body, set }) => { // Finish this after vibecheck overhaul
    ...
    // Check existing
    set.status = 400;
    if (! await Lesson.exists({ course  : targetCourse }))  
    { return "Course not found"; }
    if (! await Lesson.exists({ teacher : targetTeacher })) 
    { return "Teacher not found"; }
    if (! await Lesson.exists({ class   : targetClass }))   
...
app.patch('/lessons/', async ({ body, set }) => { // May god have mercy on my soul
    ...
    if ("" != body.newClassroom) {
        if (! await Classroom.exists( { name : body.newClassroom })) 
        { return "Suggested classroom doesn't exist."; }
        target.classroom = body.newClassroom; }
    if ("" != body.newCourse) {
        if (! await Course.exists(    { name : body.newCourse }))    
        { return "Suggested course doesn't exist."; }
        target.course = body.newCourse; }
    if ("" != body.newTeacher) {
        if (! await Teacher.exists(   { name : body.newTeacher }))   
        { return "Suggested teacher doesn't exist"; }
        target.teacher = body.newTeacher; }
    if ("" != body.newClass) {
        if (! await Group.exists(     { name : body.newClass }))     
        { return "Suggested class doesn't exist"; }
        target.class = body.newClass; }
    if (-1 != body.newWeek) {
        if (1 > body.newWeek || 52 < body.newWeek)                 
        { return "Suggested week is outside Gregorian standard."; }
        target.class = body.newClass; }
    if (-1 != body.newWeekDay) {
        if (1 > body.newWeekDay || 5 < body.newWeekDay)         
        { return "Suggested weekday is outside Gregorian standard."; }
        target.weekDay = body.newWeekDay; }
